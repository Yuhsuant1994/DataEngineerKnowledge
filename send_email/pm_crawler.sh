...
while [ $main_attempt -lt $main_retry_limit ]; do
    ...
    run_command_with_retries "pm_crawl hashtag" "pm_hashtag" &
    PID1=$!
    run_command_with_retries "pm_crawl location" "pm_location" &
    PID2=$!
    run_command_with_retries "pm_crawl user" "pm_user" &
    PID3=$!
...
if $all_success; then
    if /usr/bin/python3 /Users/user/nwd_ig_crawler/sdc/ro.py pm_upload >> "${LOG_PATH}/${DATE}pm_upload.log" 2>&1; then
        /usr/bin/python3 /Users/user/nwd_ig_crawler/scheduler/send_email.py --topic "${TOPIC}" --subtopic "success"
    else
        echo "pm_upload failed. Check ${LOG_PATH}/${DATE}pm_upload.log for details." >> "${LOG_PATH}/${DATE}pm_upload.log"
